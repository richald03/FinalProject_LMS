...
$query = "SELECT * FROM scheduled_events WHERE id = ? AND admin_id = ?";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title) || empty($data) || empty($time) || empty($end_time) || empty($description)) {
        echo "All fields are required.";
    } else {
        // Prepare the update query
        $update_query = "UPDATE scheduled_events SET title = ?, data = ?, time = ?, end_time = ?, description = ? WHERE id = ? AND admin_id = ?";
        $stmt = $conn->prepare($update_query);

        // Check if the description is being updated properly
        if ($stmt) {
            $stmt->bind_param("ssssssi", $title, $data, $time, $end_time, $description, $event_id, $admin_id);
    ...
    <div class="container">
        ...
        <form method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="date" name="data" id="data" class="form-control" value="<?php echo htmlspecialchars($event['data']); ?>" required>
            </div>
            <div class="mb-3">
