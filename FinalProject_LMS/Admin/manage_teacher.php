...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_teacher'])) {
    ...
    if ($stmt->execute()) {
        ...
        // Redirect to manage_teacher.php after deletion
...
$total_query = "SELECT COUNT(*) FROM users WHERE user_type='teacher'";
$params = [];
$types = '';

if ($search_query) {
    $total_query .= " AND (first_name LIKE ? OR last_name LIKE ? OR email LIKE ? OR employee_id LIKE ?)";
    $params = array_merge($params, [$search_term, $search_term, $search_term, $search_term]);
    $types .= 'ssss';
}

if ($filter_query) {
    $total_query .= " AND department = ?";
    $params[] = $filter_query;
    $types .= 's';
}
...
<div class="right-panel">
    ...
    <form class="search-form d-flex mt-4" method="GET" action="">
        ...
        <select name="filter" class="form-control ms-2" onchange="this.form.submit()">
            ...
            <?php
            $departments = $conn->query("SELECT DISTINCT department_name FROM departments");
            while ($department = $departments->fetch_assoc()): ?>
                <option value="<?= htmlspecialchars($department['department_name']) ?>"<?= ($filter_query == $department['department_name']) ? ' selected' : '' ?>><?= htmlspecialchars($department['department_name']) ?></option>
            <?php endwhile; ?>
        </select>
        <a href="register.php" class="btn btn-primary ms-2">Add Teacher</a>
    </form>
    <!-- Table -->
            ...
            <tbody>
                <?php while ($row = $result->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td>
                            <a href="view_teacher.php?id=<?= $row['id'] ?>" class="btn btn-info btn-sm">View</a>
