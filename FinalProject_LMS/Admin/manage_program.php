...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_program'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO programs (program_name, program_description, department_id) VALUES (?, ?, ?)");
    $stmt->bind_param("ssi", $name, $description, $department_id);

    if ($stmt->execute()) {
        header("Location: manage_program.php"); // Redirect to the same page to reflect the new program
        exit();
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_program'])) {
    ...
    if ($stmt->execute()) {
        ...
        $conn->query("ALTER TABLE programs AUTO_INCREMENT = 1");

        header("Location: manage_program.php"); // Redirect to the same page to reflect the changes
        exit();
    } else {
        echo "Error: " . $stmt->error;
    }  
...
if (isset($_GET['search'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM programs WHERE program_name LIKE ? OR program_description LIKE ? ORDER BY id ASC LIMIT ?, ?");
    $search_term = "%" . $search_query . "%";
    $stmt->bind_param("ssii", $search_term, $search_term, $start_from, $limit);
} else {
    $stmt = $conn->prepare("SELECT * FROM programs ORDER BY id ASC LIMIT ?, ?");
...
$total_query = $conn->prepare("SELECT COUNT(*) FROM programs WHERE program_name LIKE ? OR program_description LIKE ?");
$search_term = "%" . $search_query . "%";  // Added this line to fix search_term missing issue
...
<div class="container-fluid">
    ...
    <div class="right-panel">
        ...
        <form method="POST" action="">
            ...
            <button type="submit" name="create_program" class="btn btn-primary">Create</button>
        </form>
            ...
            <tbody>
                <?php while ($row = $result->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td><?= htmlspecialchars($row['program_description']) ?></td>
                        <td><?= htmlspecialchars($row['department_id']) ?></td>
                        <td><?= htmlspecialchars($row['created_at']) ?></td>
                        <td>
                            <a href="view_program.php?id=<?= $row['id'] ?>" class="btn btn-info btn-sm">View</a>
